...
interface EndDesignDialogProps {
  sessionId: string;
  onClose: () => void;
}

export function EndDesignDialog({ sessionId, onClose }: EndDesignDialogProps) {
  ...
  const confirmEnd = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/design/abandon", {
        method: "POST",
    ...
    >
      <div className="panel w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="t-body-light text-sm text-text-muted mb-5">
          This design session will be marked ended and can&apos;t be resumed.
          It still counts toward your monthly cap. You won&apos;t get a debrief —
          those only come from finishing the loop.
        </p>
